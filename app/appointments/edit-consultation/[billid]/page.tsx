...
export default function EditConsultationPage() {
  ...
  const [consultationTypeLabel, setConsultationTypeLabel] =
    useState<string>("");
      ...
      try {
        ...
        if (bill.type !== "consultation") {
          ...
          toast({
            ...
            description: "Only consultation bills can be edited.",
            variant: "destructive",
          });
          router.push("/appointments");
          return;
        ...
        if (bill.status === "paid") {
          console.error("Bill already paid");
          toast({
            title: "Bill already paid",
            description:
              "This bill has already been paid and cannot be edited. Please create an upgrade/downgrade bill instead.",
            variant: "destructive",
          });
          router.push(`/appointments/upgrade-consultation/${billId}`);
          return;
        }

        // Check if doctor can be changed
        const canChange = canChangeDoctor(billId);
        setCanEdit(canChange);

        if (!canChange) {
          setErrorMessage(
            "This consultation cannot be edited because the patient has already seen the doctor."
          );
        }
  ...
  }, [selectedDoctor, getConsultationFeeByDoctorId]);

  // Find the handleSaveChanges function and update it to properly handle the workflow
  const handleSaveChanges = async () => {
    ...
    if (!canEdit) {
      toast({
        title: "Cannot edit consultation",
        description:
          "This consultation cannot be edited because the patient has already seen the doctor.",
        variant: "destructive",
      });
    ...
    try {
      ...
      if (result.success) {
        ...
        // Always redirect back to appointments
        router.push("/appointments");
      } else {
        toast({
          title: "Error",
      ...
      <div className="space-y-6">
        ...
        {!canEdit && (
          <Alert variant="destructive">
            ...
            <AlertTitle>Cannot edit consultation</AlertTitle>
            <AlertDescription>
              {errorMessage ||
                "This consultation cannot be edited because the patient has already seen the doctor."}
            </AlertDescription>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Edit the doctor and consultation type for this appointment
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            <div className="p-4 bg-gray-50 rounded-md space-y-2">
              ...
              <div className="flex justify-between text-sm">
                ...
                >
                  {newPrice > originalPrice
                    ? `+₦${(newPrice - originalPrice).toLocaleString()}`
                    : newPrice < originalPrice
                    ? `-₦${(originalPrice - newPrice).toLocaleString()}`
                    : "₦0"}
                </span>
              </div>
            </div>
          ...
          <CardFooter className="flex justify-between">
            ...
            <Button
              ...
              disabled={
                isSubmitting || !selectedDoctor || !consultationType || !canEdit
              }
            >
